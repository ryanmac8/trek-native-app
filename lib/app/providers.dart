...
import '../todos/todo_api.dart';
import '../todos/todo_local_store.dart';
import '../todos/todo_repository.dart';
import 'router.dart';
...
final todoApiProvider = Provider<TodoApi>((ref) {
  return TodoApi(apiClient: ref.watch(apiClientProvider));
});

/// Local (non-secure) cache of each trip's todo items — see
/// docs/offline-first.md. A minimal, scoped-to-todos stand-in for the full
/// local-persistence mechanism issue #21 will decide on for the whole data
/// model.
final todoLocalStoreProvider = Provider<TodoLocalStore>(
  (ref) => PreferencesTodoLocalStore(),
);

final todoRepositoryProvider = Provider<TodoRepository>((ref) {
  return TodoRepository(
    todoApi: ref.watch(todoApiProvider),
    localStore: ref.watch(todoLocalStoreProvider),
  );
});
