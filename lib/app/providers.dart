...
import '../packing/packing_api.dart';
import '../packing/packing_local_store.dart';
import '../packing/packing_repository.dart';
import 'router.dart';
...
final packingApiProvider = Provider<PackingApi>((ref) {
  return PackingApi(apiClient: ref.watch(apiClientProvider));
});

/// Local (non-secure) cache of each trip's packing items — see
/// docs/offline-first.md. A minimal, scoped-to-packing stand-in for the
/// full local-persistence mechanism issue #21 will decide on for the whole
/// data model.
final packingLocalStoreProvider = Provider<PackingLocalStore>(
  (ref) => PreferencesPackingLocalStore(),
);

final packingRepositoryProvider = Provider<PackingRepository>((ref) {
  return PackingRepository(
    packingApi: ref.watch(packingApiProvider),
    localStore: ref.watch(packingLocalStoreProvider),
  );
});
