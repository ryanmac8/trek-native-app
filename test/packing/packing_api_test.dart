...
PackingApi _api(http.Client httpClient) {
  return PackingApi(
    apiClient: ApiClient(
    ...
    test('GETs the trip-scoped endpoint and parses the wrapped list', () async {
      ...
      final api = _api(
        MockClient((request) async {
          ...
          return _json({
            'items': [
              {'id': 1, 'trip_id': 20, 'name': 'Hiking boots', 'checked': 0},
            ],
          });
        }),
      );
      ...
      expect(requestedUri?.path, '/api/trips/20/packing');
      expect(items.single.name, 'Hiking boots');
      expect(items.single.tripId, '20');
    });

    test(
      'an empty trip packing list returns an empty list, not an error',
      () async {
  ...
  group('createItem', () {
    test(
      ...
      () async {
        ...
        expect(sentBody, {'name': 'Passport', 'category': 'Documents'});
        expect(item.id, 7);
    ...
    test('omits optional fields entirely when not provided', () async {
      ...
      await api.createItem('20', name: 'Passport');
