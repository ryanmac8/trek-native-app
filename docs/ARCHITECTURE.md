...
- **Budget** ([#7](https://github.com/ryanmac8/trek-native-app/issues/7)) — the trip dashboard's Budget tab lists and creates budget items offline-first against a trip-scoped local cache (`BudgetRepository`). See [budget.md](budget.md). Open: item edit/delete/reorder, per-person splits/payers, settlements, and category management.
- **Offline-first sync mechanism** ([#21](https://github.com/ryanmac8/trek-native-app/issues/21)) — local database choice, conflict resolution strategy, and write-retry/outbox mechanism are undecided. See [offline-first.md](offline-first.md). Budget has a minimal, `shared_preferences`-backed version of this scoped to its own resource (see [budget.md](budget.md)) as a first cut; it doesn't generalize to other resources yet.
