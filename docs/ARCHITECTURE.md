...
- **Packing** ([#8](https://github.com/ryanmac8/trek-native-app/issues/8)) — the trip dashboard's Packing tab lists and creates packing items offline-first against a trip-scoped local cache (`PackingRepository`). See [packing.md](packing.md). Open: checking items off, delete/reorder, bags, sharing, and templates.
- **Offline-first sync mechanism** ([#21](https://github.com/ryanmac8/trek-native-app/issues/21)) — local database choice, conflict resolution strategy, and write-retry/outbox mechanism are undecided. See [offline-first.md](offline-first.md). Packing has a minimal, `shared_preferences`-backed version of this scoped to its own resource (see [packing.md](packing.md)) as a first cut; it doesn't generalize to other resources yet.
