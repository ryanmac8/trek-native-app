...
- **Collaboration — notes** ([#13](https://github.com/ryanmac8/trek-native-app/issues/13)) — the trip dashboard's Notes tab lists and creates shared collab notes offline-first against a trip-scoped local cache (`CollabRepository`). See [collaboration.md](collaboration.md). Open: editing, deleting, pinning from the client, file attachments, member list/add/remove, polls, and chat.
- **Offline-first sync mechanism** ([#21](https://github.com/ryanmac8/trek-native-app/issues/21)) — local database choice, conflict resolution strategy, and write-retry/outbox mechanism are undecided. See [offline-first.md](offline-first.md). Collab notes has a minimal, `shared_preferences`-backed version of this scoped to its own resource (see [collaboration.md](collaboration.md)) as a first cut; it doesn't generalize to other resources yet.
