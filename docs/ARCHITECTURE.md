...
- **Collaboration — notes** ([#13](https://github.com/ryanmac8/trek-native-app/issues/13)) — the trip dashboard's Notes tab lists, creates, edits, and deletes shared collab notes offline-first against a trip-scoped local cache (`CollabRepository`). See [collaboration.md](collaboration.md). Open: pinning from the client, file attachments, member list/add/remove, polls, and chat.
