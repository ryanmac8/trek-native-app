...
- **Accommodations — list** ([#10](https://github.com/ryanmac8/trek-native-app/issues/10)) — the trip dashboard's Stays tab lists a trip's accommodations offline-first against a trip-scoped local cache (`AccommodationsRepository`). See [accommodations.md](accommodations.md). Open: create/edit/delete, the check-in/check-out day-range picker, linking a place as an accommodation, and itinerary-day context.
- **Offline-first sync mechanism** ([#21](https://github.com/ryanmac8/trek-native-app/issues/21)) — local database choice, conflict resolution strategy, and write-retry/outbox mechanism are undecided. See [offline-first.md](offline-first.md). The Stays tab has a minimal, `shared_preferences`-backed version of this scoped to its own (read-only) resource as a first cut; it doesn't generalize to other resources yet.
