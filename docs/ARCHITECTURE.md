...
- **State management, navigation, design system** ([#2](https://github.com/ryanmac8/trek-native-app/issues/2)) — first slice done: Riverpod for state/DI, go_router for navigation (auth/server-gated redirects), and a Material 3 design system (colors, typography, spacing, Trek's place category colors). See [app-shell.md](app-shell.md). Open: local caching strategy, global error/toast system, app icon/splash, and the rest of the reusable UI kit.
