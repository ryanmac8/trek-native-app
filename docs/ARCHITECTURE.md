...
- **Reservations — list** ([#11](https://github.com/ryanmac8/trek-native-app/issues/11)) — the trip dashboard's Bookings tab lists a trip's reservations offline-first against a trip-scoped local cache (`ReservationsRepository`). See [reservations.md](reservations.md). Open: create/edit/delete, the day/place picker, multi-leg transport detail, and travelers.
- **Offline-first sync mechanism** ([#21](https://github.com/ryanmac8/trek-native-app/issues/21)) — local database choice, conflict resolution strategy, and write-retry/outbox mechanism are undecided. See [offline-first.md](offline-first.md). The Bookings tab has a minimal, `shared_preferences`-backed version of this scoped to its own (read-only) resource as a first cut; it doesn't generalize to other resources yet.
