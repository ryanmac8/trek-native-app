...
- **Sharing — trip share link** ([#14](https://github.com/ryanmac8/trek-native-app/issues/14)) — a share icon on the trip dashboard lets a trip's owner/editor enable a public, read-only link, choose which sections it exposes, and turn it off, offline-first against a trip-scoped local cache (`ShareRepository`). See [sharing.md](sharing.md). Open: export (PDF/ICS), the public `/shared/:token` viewing page itself, and token rotation.
- **Offline-first sync mechanism** ([#21](https://github.com/ryanmac8/trek-native-app/issues/21)) — local database choice, conflict resolution strategy, and write-retry/outbox mechanism are undecided. See [offline-first.md](offline-first.md). Trip sharing has a minimal, `shared_preferences`-backed version of this scoped to its own resource (see [sharing.md](sharing.md)) as a first cut; it doesn't generalize to other resources yet.
