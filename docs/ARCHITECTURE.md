...
- **Todos** ([#9](https://github.com/ryanmac8/trek-native-app/issues/9)) — the trip dashboard's Todos tab lists, creates, checks/unchecks, deletes, and reorders todo items offline-first against a trip-scoped local cache (`TodoRepository`). See [todos.md](todos.md). Open: due dates, description, assignment, priority, and category assignees.
