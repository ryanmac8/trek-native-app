...
- **Networking & auth** ([#1](https://github.com/ryanmac8/trek-native-app/issues/1)) — HTTP client, error handling, environment config, secure token storage, and the login/MFA/logout flow (verified against Trek's actual backend, which issues one JWT with no refresh — see [networking-auth.md](networking-auth.md)) are implemented. Still open: login UI (blocked on #2), full request/response models for the rest of the data model, wiring the auth flow into the app shell, and whether "environment" should mean a user-entered self-hosted server URL rather than fixed dev/staging/prod constants (Trek is self-hosted, not a vendor-run API).
