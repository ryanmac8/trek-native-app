...
Covers the list + create first slice of [issue #9](https://github.com/ryanmac8/trek-native-app/issues/9): the Todos tab of the trip dashboard, backed by a trip-scoped local cache. Checking items off, delete/reorder, due dates, description, assignment, priority, and category assignees are not built yet.
...
| [lib/todos/todo_api.dart](../lib/todos/todo_api.dart) | `TodoApi` — thin wrapper around `GET /api/trips/:tripId/todo` and `POST /api/trips/:tripId/todo`. |
...
The Todos tab renders a still-pending item with a "Syncing…" subtitle and a sync icon instead of its usual checked/unchecked icon.

## What's still open

- Checking/unchecking an item and item delete/reorder (the rest of `TodoApi`/`TodoRepository` for issue #9).
